...
function buildPortabilityProcessSpeech(input: Block6PortabilitySpeechInput): string {
  ...
  const parts = [
    ...
    "",
    "Mientras tu número no se porte a WOM, activaremos el servicio con un número temporal, que es un número distinto al que estás portando. Este número será solo temporal hasta que podamos portar tu número real. Lo importante es que, mientras tu número no se porte, no perderás el servicio con tu operador; por eso es importante que no cambies el chip hasta que estés portado.",
    "",
