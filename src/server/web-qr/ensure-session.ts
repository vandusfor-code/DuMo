...
export async function ensureWebQrBridgeReady(channelId: string): Promise<void> {
  ...
  if (!channel) throw new Error("Canal WhatsApp Web no encontrado.");

  const sessionId = bridgeSessionId(channelId);
  ...
  for (let i = 0; i < 25; i++) {
    ...
    if (status?.status === "CONNECTED") {
      ...
      await webQrRepository.updateSessionBridge({ channelId, bridgeSessionId: sessionId });
      return;
    }
