...
import { useAdminSaleDetail } from "@/hooks/use-admin-sales";

export default function AdminSaleDetailPage() {
  const params = useParams<{ id: string }>();
  const id = params.id;
  const { data, isLoading, isFetching } = useAdminSaleDetail(id);
      ...
      <div className="space-y-6 pt-1">
        ...
        <Card className="grid place-items-center px-6 py-20 text-center">
          ...
          <Button asChild variant="outline" className="mt-6">
            <Link href="/admin/ventas">Ir a Ventas</Link>
          </Button>
        </Card>
      </div>
    );
  ...
  return <SaleDetailView sale={data} backHref="/admin/ventas" backLabel="Volver a Ventas" />;
}
